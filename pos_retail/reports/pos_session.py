...
class pos_session(models.Model):
    ...
    def get_session_date(self, date_time):
        if date_time:
            ...
            if sign == '+':
                if version_info != 12:
                    date_time = datetime.strptime(date_time, DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(hours=hour_tz,
                                                                                                         minutes=min_tz)
                else:
    ...
    def get_current_date(self):
        ...
        if tz:
            report_date = timezone(tz).localize(start_dt).astimezone(UTC).date()
        else:
    ...
    def get_current_time(self):
        ...
        start_dt = datetime.utcnow()
        if tz:
            report_time = timezone(tz).localize(start_dt).astimezone(UTC).time().strftime("%H:%M:%S")
        else:
